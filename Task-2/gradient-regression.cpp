...
GradientRegression::GradientRegression(double learning_rate, int num_epochs)
...
MultiGradientRegression::MultiGradientRegression(int num_features, double learning_rate, int num_epochs)
...
double MultiGradientRegression::compute_loss(const std::vector<DataPoints>& predictions, const std::vector<DataPoints>& dataset) {
    ...
    for (long i = 0; i < dataset.size(); ++i) {
        ...
        total_loss += (diff_y1 * diff_y1 + diff_y2 * diff_y2) / 2.0;
    }
